...
| `parcels-compact.geojson` | GeoJSON FeatureCollection | Web-optimized: one **Point** per parcel (centroid via `representative_point` — EPIC-LA case point preferred, DINS polygon centroid fallback) carrying only the attributes the frontend map reads (`ain`, `address`, `sfr_size_bucket`, `lfl_bucket`, `adu_bucket`, `adds_sb9`, `adds_sb1123`, the seven `rebuild_*` milestone booleans, and `rebuild_stage`). The per-parcel bucket keys / milestone flags come from the same classifiers that produce `summary.json`'s counts, so the map can never drift from the summary. |
